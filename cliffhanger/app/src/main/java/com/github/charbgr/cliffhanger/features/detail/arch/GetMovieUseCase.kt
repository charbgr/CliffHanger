...
import com.github.charbgr.cliffhanger.network.tmdb.entity.FullMovieEntityMapper
...
internal class GetMovieUseCase(
  ...
  private val mapper = FullMovieEntityMapper()

  override fun build(params: Int): Observable<PartialChange> {
    return movieRepository.getMovie(params)
        .toObservable()
        .map { PartialChange.Success(mapper.transform(it)) as PartialChange }
